...
tempat_uang = tk.Frame(root, bg="white")
tempat_uang.pack(pady=5, fill="x")
tk.Label(tempat_uang, text="Masukkan Uang anda:", bg="#f0f0f0").pack(side="left", padx=10)

entry_uang = tk.Entry(tempat_uang, width=8)
entry_uang.pack(side="left", padx=5)

btn_uang = tk.Button(tempat_uang, text="Masukkan", bg="lightblue", command=lambda: tambah_uang())
btn_uang.pack(side="left", padx=5)
...
def show_struk(produk, kembalian):
    jendela_struk = tk.Toplevel(root)
    jendela_struk.title("Struk Pembelian")
    jendela_struk.geometry("300x450")
    nama_file = produk["file"]
    gambar = simpan_gambar.get(nama_file)

    if gambar:
        tk.Label(jendela_struk, image=gambar).pack(pady=20)

    info_transaksi = f"Barang yang dibeli:\n{produk['nama']}\n\nKembalian Anda:\nRp {kembalian:,}"  
    tk.Label(jendela_struk, text=info_transaksi, font=("arial", 12, "bold")).pack(pady=10)
    tk.Button(jendela_struk, text="Ambil", bg="green", fg="white", command=jendela_struk.destroy).pack(pady=20)
...
def tampilkan_etalase(nomor_halaman):
    ...
    indeks_awal = nomor_halaman * 9
    indeks_akhir = indeks_awal + 9
    ...
    for item in halaman_tampil:
        ...
        kotak_produk = tk.Frame(etalase, bd=2, relief="ridge", bg="white")
        kotak_produk.grid(row=baris, column=kolom, padx=2, pady=2)
